...
func TestSessionAdapterBasic(t *testing.T) {
	...
	// 创建内存store
	store := session.NewMemoryStore("test_session_123")
	adapter := session.NewAdapter(store, nil)
...
func TestSessionAdapterWithNilStore(t *testing.T) {
	...
	// 测试nil store的处理
	adapter := session.NewAdapter(nil, nil)
...
func TestContextExtensionBasic(t *testing.T) {
	...
	// 创建Hertz上下文扩展
	mockRequest := &app.RequestContext{}
	ext := session.NewExtensionForHertzContext(mockRequest)
...
func BenchmarkSessionAdapterOperations(b *testing.B) {
	store := session.NewMemoryStore("bench_session")
	adapter := session.NewAdapter(store, nil)
