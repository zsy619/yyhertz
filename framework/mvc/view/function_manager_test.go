...
func TestTemplateFunctionManager(t *testing.T) {
	...
	// 测试内置函数是否正确加载
	builtinCount := GetBuiltinFunctionCount()
	if builtinCount == 0 {
	...
	if source != FunctionSourceGlobal {
		t.Errorf("Expected source to be global, got %s", source)
	}

	// 测试内置函数来源
	if HasBuiltinFunction("add") {
		source = manager.GetFunctionSource("add")
		if source != FunctionSourceBuiltin {
...
func TestBackwardCompatibility(t *testing.T) {
	...
	AddGlobalFunction("oldTestFunc", oldTestFunc)
	...
	if _, exists := TemplateFuncs["add"]; !exists {
