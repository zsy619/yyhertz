...
{{end}}
	...
	// 测试模板加载
	result, err := view.LoadTemplate(templatePath, data)
	if err != nil {
...
<p>{{.Content}}</p>{{end}}`
	...
	// 测试带布局的模板加载
	result, err := view.LoadTemplateWithLayout(layoutPath, contentPath, data)
	if err != nil {
...
func TestTemplateFunctions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			fn, exists := view.TemplateFuncs[tt.function]
			if !exists {
...
func TestMakeSlice(t *testing.T) {
	result := view.MakeSlice("a", 1, true)
...
func TestConcatString(t *testing.T) {
	result := view.ConcatString("Hello", " ", "World", "!")
...
func TestContainString(t *testing.T) {
	...
	for _, tt := range tests {
		result := view.ContainString(tt.s, tt.substr)
...
func TestAuthContain(t *testing.T) {
	...
	for _, tt := range tests {
		result := view.AuthContain(tt.s, tt.value)
		if result != tt.expected {
...
func TestLogicalFunctions(t *testing.T) {
	tests := []struct {
		...
		{"And true true", view.And(true, true) == true},
		{"And true false", view.And(true, false) == false},
		{"Or true false", view.Or(true, false) == true},
		{"Or false false", view.Or(false, false) == false},
		{"Not true", view.Not(true) == false},
		{"Not false", view.Not(false) == true},
	}
...
func TestCollectionFunctions(t *testing.T) {
	...
	// Index
	if view.Index(slice, 1) != "b" {
		t.Error("Index(slice, 1) should return 'b'")
	}

	// Slice
	result := view.Slice(slice, 1, 3)
