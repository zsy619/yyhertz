...
# ============= 多节点数据库配置示例 =============
# 使用 GetDatabasePrimary(nodeName) 函数可动态获取以下配置节点
# 支持与 primary 节点相同的所有配置选项
...
# 使用示例:
# config, err := GetDatabasePrimary("sso")        // 获取SSO数据库配置
# config, err := GetDatabasePrimary("user")       // 获取用户数据库配置  
# config, err := GetDatabasePrimary("order")      // 获取订单数据库配置
# dsn, err := GetDatabaseDSN("sso")               // 获取SSO数据库DSN连接字符串
# nodes := GetAllDatabaseNodes()                  // 获取所有配置的数据库节点列表
