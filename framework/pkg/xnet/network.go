	...
	for key, value := range data {
		...
		case []any:
			for i, item := range v {
				...
				parts = append(parts, itemKey+"="+url.QueryEscape(fmt.Sprintf("%v", item)))
			}
		default:
			parts = append(parts, fullKey+"="+url.QueryEscape(fmt.Sprintf("%v", value)))
		}
	}
...
func ParseStr(str string) map[string]any {
	...
	for _, pair := range pairs {
		if idx := strings.Index(pair, "="); idx != -1 {
			...
			if decodedKey, err := url.QueryUnescape(pair); err == nil {
				result[decodedKey] = ""
			} else {
				result[pair] = ""
			}
