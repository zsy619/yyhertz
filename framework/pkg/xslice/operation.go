...
type ReduceType func(any) any
type FilterType func(any) bool
...
func InSliceIface(v any, sl []any) bool {
	for _, vv := range sl {
		if vv == v {
			return true
		}
...
func ArrayKeyExists(key any, arr any) bool {
	v := reflect.ValueOf(arr)
	switch v.Kind() {
	case reflect.Map:
...
func SliceMerge(slice1, slice2 []any) []any {
...
func SliceUnique(slice []any) []any {
	var uniqueslice []any
	for _, v := range slice {
...
func SliceFilter(slice []any, a FilterType) []any {
	var ftslice []any
	for _, v := range slice {
		if a(v) {
...
func SliceReduce(slice []any, a ReduceType) []any {
	var dslice []any
	for _, v := range slice {
...
// ArrayReduce applies callback function against an accumulator and each element
func ArrayReduce(arr []any, initial any, callback func(any, any) any) any {
	result := initial
	for _, v := range arr {
...
func SliceRange(start, end, step int64) []int64 {
	var intslice []int64
...
func SliceShuffle(slice []any) []any {
	rand.Seed(time.Now().UnixNano())
...
func SliceDiff(slice1, slice2 []any) []any {
	var diffslice []any
...
func SliceIntersect(slice1, slice2 []any) []any {
	var diffslice []any
	for _, v := range slice1 {
		if InSliceIface(v, slice2) {
			diffslice = append(diffslice, v)
...
func SlicePad(slice []any, size int, val any) []any {
	if size <= len(slice) {
		return slice
	}
...
// ArrayFlatten flattens a multi-dimensional array
...
// SortInts sorts a slice of integers
...
// SortFloat64s sorts a slice of float64s
