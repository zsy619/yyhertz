...
// JsonPretty 格式化输入的 JSON 字符串，使其具有可读性。
...
// JsonMinify 将输入的 JSON 字符串进行最小化处理，去除不必要的空格和换行符。
...
// JsonpEncode 将给定的数据编码为 JSON 字符串，并包装成回调函数的调用形式。
//
// 参数：
//   - data: 需要编码为 JSON 的数据。
...
// 示例：
//
//	result := JsonpEncode("callbackFunc", map[string]string{"key": "value"})
