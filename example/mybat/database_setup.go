...
func (setup *DatabaseSetup) CreateTestDatabase() error {
	config := DefaultDatabaseConfig()
	systemDSN := config.BuildSystemDSN()
	
	// 连接到MySQL系统数据库
	db, err := sql.Open("mysql", systemDSN)
	if err != nil {
	...
	// 创建测试数据库
	createSQL := fmt.Sprintf("CREATE DATABASE IF NOT EXISTS `%s` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci", setup.TestDBName)
	_, err = db.Exec(createSQL)
	if err != nil {
		return fmt.Errorf("failed to create test database: %w", err)
	}
	
	log.Printf("测试数据库 '%s' 创建成功", setup.TestDBName)
	return nil
}
...
func (setup *DatabaseSetup) DropTestDatabase() error {
	...
	// 删除测试数据库
	dropSQL := fmt.Sprintf("DROP DATABASE IF EXISTS `%s`", setup.TestDBName)
	_, err = db.Exec(dropSQL)
	if err != nil {
		return fmt.Errorf("failed to drop test database: %w", err)
	}
	
	log.Printf("测试数据库 '%s' 删除成功", setup.TestDBName)
	return nil
}
...
func (setup *DatabaseSetup) ConnectToDatabase() error {
	...
	db, err := gorm.Open(mysql.Open(setup.DSN), gormConfig)
	if err != nil {
