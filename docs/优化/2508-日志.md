...
## 添加日志输出格式
在文件中 @framework/config/log_formatters.go 添加logrus对gin、Iris、Ent、go-zero、Fiber、Echo、Revel、Buffalo等框架的formatter
- 在 @framework/config/log.go 中 完善LogLevel、LogFormat 
- 在 @framework/config/log_formatters.go 中 完善调用GetFormatter
- 查询 @framework/config/ 下 log 开头的文件，确保所有日志相关的配置和格式化都得到一致的处理
